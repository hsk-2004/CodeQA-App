...
<ul>
  <li><b>App / Orchestration Service</b> (<code>app_service.py</code>, port 8000) &mdash; the "receptionist".
      This is the only file the user ever directly talks to. It does not answer questions itself &mdash;
      it coordinates: asks the Retrieval Service what code is relevant, sends that context + the question
      to Code Llama, and returns the final answer to the user.</li>
  <li><b>Retrieval Service</b> (<code>retrieval_service.py</code>, port 8001) &mdash; the "librarian".
      Its only job is search: given a question, it converts it into an embedding, compares it against
      every chunk in the knowledge base, and returns the top 2 most relevant chunks.</li>
</ul>

<h3>Request Flow, Step by Step</h3>
<div class="box">
<ol>
  <li>User sends a question to <code>http://localhost:8000/ask</code> &rarr; lands in <code>app_service.py</code>.</li>
  <li><code>app_service.py</code> immediately calls <code>http://localhost:8001/retrieve</code> &rarr; lands in <code>retrieval_service.py</code>.</li>
  <li><code>retrieval_service.py</code> embeds the question, compares it against <code>knowledge_base.json</code>,
      and picks the top 2 most relevant chunks.</li>
  <li>Those chunks are sent back to <code>app_service.py</code>.</li>
  <li><code>app_service.py</code> combines the question + retrieved chunks into one prompt and sends it to
      Ollama (<code>http://localhost:11434</code>) to be answered by Code Llama.</li>
  <li>Code Llama's answer is returned to <code>app_service.py</code>.</li>
  <li><code>app_service.py</code> sends that final answer back to the user.</li>
</ol>
</div>
...
<p><code>app_service.py</code> is the only entry point the user calls; it makes two outgoing calls
of its own (one to Retrieval, one to the LLM) and stitches the results together before replying &mdash;
this is what "orchestration" means in the assignment's requirements.</p>
